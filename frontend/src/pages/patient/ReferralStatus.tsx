  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-ink">Referral Status</h1>
          <p className="text-sm text-ink-soft">Track your specialist referral journey</p>
        </div>
      </div>
      ...
      {referrals.length === 0 ? (
        <div className="text-center py-16 text-ink-soft">
          ...
          <p className="font-semibold text-ink-muted">No active referrals</p>
          ...
          {referrals.map(referral => {
            ...
            return (
              <Card key={referral.id} className="overflow-hidden">
                ...
                <div className="p-5 border-b border-line">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-bold text-ink">Referral #{referral.referralCode}</p>
                      ...
                      <p className="text-xs text-ink-soft mt-0.5">{referral.referringFacilityName}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-ink-soft">Created</p>
                      <p className="text-sm font-semibold text-sand-700">{(referral.createdAt ?? '').split('T')[0]}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs text-ink-soft">Referred to</p>
                        <p className="text-sm font-semibold text-sand-700">{referral.targetFacilityName}</p>
                      </div>
                    </div>
                    {referral.assignedSpecialistName && (
                      <div className="flex items-start gap-2">
                        <User className="w-4 h-4 text-ink-soft mt-0.5 shrink-0" />
                        <div>
                          <p className="text-xs text-ink-soft">Assigned Specialist</p>
                          <p className="text-sm font-semibold text-sand-700">{referral.assignedSpecialistName}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {/* Timeline */}
                <div className="p-5">
                  <h3 className="text-sm font-bold text-sand-700 mb-4">Journey Tracker</h3>
                ...
                <div className="px-5 pb-5">
                  <div className="bg-sand-50 rounded-xl p-4">
                    <p className="text-xs font-bold text-ink-muted mb-1">Clinical Summary (from your doctor)</p>
                    <p className="text-sm text-sand-700">{referral.clinicalSummary}</p>
                  </div>
                </div>
              </Card>
            );
