...
export const PatientLabReports: React.FC = () => {
  ...
  const getTrend = (isAbnormal?: boolean) => {
    ...
    return { icon: <Minus className="w-4 h-4 text-slate-400" />, label: 'Pending' };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Your diagnostic test results and orders</p>
        </div>
      </div>
            ...
            <Card key={order.id} className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Ordered: {order.dateOrdered}</p>
                </div>
              ...
              {isCompleted && order.result && (
                <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                      <p className="text-xs text-slate-400 font-medium">Result</p>
                      <p className={`font-bold text-lg mt-0.5 ${order.isAbnormal ? 'text-red-600' : 'text-green-600'}`}>
                        {order.result}
                        {order.unit && <span className="text-sm font-normal text-slate-500 ml-1">{order.unit}</span>}
                      </p>
                    </div>
                    {order.referenceRange && (
                      <div>
                        <p className="text-xs text-slate-400 font-medium">Reference Range</p>
                        <p className="text-sm font-semibold text-slate-700 mt-0.5">{order.referenceRange}</p>
                      </div>
                    )}
                    <div>
                      <p className="text-xs text-slate-400 font-medium">Interpretation</p>
                    ...
                    {order.completedDate && (
                      <div>
                        ...
                        <p className="text-sm font-semibold text-slate-700 mt-0.5">{order.completedDate}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {order.notes && (
                    <div className="mt-3 pt-3 border-t border-slate-200">
                      <p className="text-xs text-slate-500"><span className="font-semibold">Lab Notes: </span>{order.notes}</p>
                    </div>
                  )}
                </div>
              )}
