...
export default function RoleFeaturePage() {
  ...
  return <section className="space-y-6" aria-labelledby="feature-title">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div><p className="text-sm font-semibold text-gov-700">MahaAarogya Sangam</p><h1 id="feature-title" className="text-2xl font-bold text-slate-900">{screen.title}</h1><p className="mt-1 max-w-2xl text-slate-600">{screen.description}</p></div>
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <Card className="lg:col-span-2"><CardHeader><CardTitle className="flex items-center gap-2"><Activity size={19} className="text-gov-700" />Current items</CardTitle></CardHeader><CardContent className="space-y-3">{visible.map((item) => <div key={item} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 p-4"><span className="font-medium text-slate-800">{item}</span><Badge variant="info">Active</Badge></div>)}{visible.length === 0 && <p className="py-6 text-center text-slate-500">No matching records. Try a different search.</p>}</CardContent></Card>
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck size={19} className="text-emerald-700" />Safe actions</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-slate-600">Changes are recorded in the demo audit trail and require confirmation in production.</p><Button variant="outline" className="w-full" onClick={() => setCompleted((items) => [...items, 'Review saved'])}><CheckCircle2 size={17} />Save review</Button><Button variant="outline" className="w-full" onClick={() => setCompleted((items) => [...items, 'Export prepared'])}><Download size={17} />Export view</Button>{completed.map((item) => <p key={item} className="text-xs text-emerald-700">{item}</p>)}</CardContent></Card>
    </div>
