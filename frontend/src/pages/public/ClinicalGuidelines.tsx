            ...
            return (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 font-medium">Specialty: {guide.specialty}</p>
                    </div>
                    <button
                      onClick={() => toggleBookmark(guide.id)}
                      className={`p-1.5 rounded-lg border transition-colors ${
                        isBookmarked ? 'bg-gov-50 text-gov-700 border-gov-300' : 'text-slate-400 border-slate-200 hover:text-slate-700'
                      }`}
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed line-clamp-3">{guide.summary}</p>
                ...
                <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between gap-2 text-xs">
                  <span className="text-[11px] text-slate-400">Updated: {guide.updatedDate}</span>
          ...
          <Modal
            ...
            footer={
              <div className="flex justify-between items-center w-full">
                <span className="text-[11px] text-slate-400">Version 2.4 • Effective State-Wide</span>
          ...
          >
            <div className="space-y-4 text-xs text-slate-800 leading-relaxed">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                <h5 className="font-bold text-slate-900 uppercase tracking-wider mb-1">Executive Summary:</h5>
                <p>{activeGuideline.summary}</p>
              </div>

              <div>
                <h5 className="font-bold text-slate-900 uppercase tracking-wider mb-2">Standard Diagnostic & Management Flow:</h5>
                <div className="space-y-2">
                  <div className="p-3 bg-white border border-slate-200 rounded-lg">
                    <div className="font-bold text-gov-800">1. Initial Triage & Red-Flag Assessment:</div>
                    <p className="text-slate-600 mt-0.5">Verify vital signs (BP, Pulse, SpO2, Temp, Glucose). Identify any emergent contraindications requiring immediate resuscitation.</p>
                  </div>
                  <div className="p-3 bg-white border border-slate-200 rounded-lg">
                    <div className="font-bold text-gov-800">2. First-Line Pharmacotherapy:</div>
                    <p className="text-slate-600 mt-0.5">Initiate formulary-listed medications available at the PHC pharmacy. Cross-check for recorded penicillin, sulfa, or NSAID allergies in EHR.</p>
                  </div>
                  <div className="p-3 bg-white border border-slate-200 rounded-lg">
                    <div className="font-bold text-gov-800">3. Tele-Referral Escalation Criteria:</div>
                    <p className="text-slate-600 mt-0.5">If clinical thresholds exceed primary capability, create a 108 tele-referral with clinical summary to reserve a tertiary bed at District/GMC hospital.</p>
                  </div>
                </div>
              </div>
            </div>
